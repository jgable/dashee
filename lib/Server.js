...
_.extend(DasheeServer.prototype, {
    start: function (done) {
        ...
        this.addAssetPaths("/assets/dashee", path.join(__dirname, '..', 'assets'), function (err) {
            ...
            self.getAssetsPaths(function (err, assetsPaths) {
                ...
                bayeux.attach(self.httpServer);

                self.app.set("bayeux", bayeux);
    ...
    registerHelpers: function () {
        ...
        hbs.registerHelper('dasheeJS', function () {
            ...
            return "<script src='/live/client.js'></script><script src='" + self.assetsPaths[dasheePath] + "'></script>";
        });
        ...
        hbs.registerHelper('dasheeBlockStyles', function () {
            ...
            _.each(self.assetsPaths, function (digestPath) {
                if (digestPath.slice(0, 15) === '/assets/blocks/' && digestPath.slice(-4) === '.css') {
        ...
        hbs.registerHelper('dasheeBlockScripts', function () {
            ...
            _.each(self.assetsPaths, function (digestPath) {
                if (digestPath.slice(0, 15) === '/assets/blocks/' && digestPath.slice(-3) === '.js') {
    ...
    registerSockets: function () {
        var bayeux = this.app.get('bayeux');

        this.sockets = bayeux.getClient();
    },

    sendBlockData: function (blockId, data, done) {
        if (!this.sockets) {
            return done();
        }

        this.sockets.publish("/blocks/" + blockId + "/data", data);

        // TODO: Can we determine whether the publish succeeded?
        process.nextTick(function () {
            done();
        });
    },

    sendBlockError: function (blockId, error) {
        if (!this.sockets) {
            return;
        }
        
        this.sockets.publish("/blocks/" + blockId + "/error", error);
    },
    ...
    getAssetPath: function (root, path, done) {
        ...
        done(null, found.digestPath || path);
    },
    ...
    addAssetPaths: function (root, paths, done) {
        ...
        if (!this.environments[root]) {
            ...
            environment.registerHelper('dasheeBlocksObject', function () {
                return self.getBlocksMapping(self.blocks);
            });

            environment.registerHelper('dasheeSocketsUrl', function () {
                return "http://" + self.config.host + ":" + self.config.port + "/live";
            });
    ...
    getBlocksMapping: function (blocks) {
        // Build up the id: name pairs
        var mappings = _.map(blocks, function (block) {
...
DasheeServer.DEFAULTS = {
    ...
    socketsPort: process.env.SOCKETS_PORT || 8080,
