...
function DasheeBlock(name, config) {
    ...
    this.config.id = config.id || uuid.v4();
    ...
    load: function (dashee, done) {
        ...
        try {
            ...
            try {
                ...
                fullPathE.message += " -> Unable to require the block with name or path: " + this.name + ".  Tried by name and " + fullPathTry;
                return done(fullPathE);
            }
        }
